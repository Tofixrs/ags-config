...
class WifiPassword extends Service {
	...
	async connectWifi(bssid: string, ssid: string) {
		this.bssid = bssid;
		this.ssid = ssid;
		const connected = !(await execAsync(`nmcli device wifi connect ${this.bssid}`).catch(err => err)).startsWith("Error");
	...
	async connectWithPassword(password: string) {
		...
		const connected = !(await execAsync(`nmcli d w connect ${this.bssid} password ${this.password}`).catch((err) => err)).startsWith("Error");
		if (connected) return this.flush()
